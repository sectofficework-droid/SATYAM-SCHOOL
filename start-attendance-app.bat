...
if not exist "lib\main_attendance.dart" (
  echo [ERROR] lib\main_attendance.dart not found - is mobile-app in the right place?
  pause
  exit /b 1
)
...
echo Starting Attendance Kiosk app (Flutter Web) on http://localhost:5002 ...
echo NOTE: face-scan needs a camera and works best on a real device, not a browser.
call "%FLUTTER%" run -t lib\main_attendance.dart -d chrome --web-port=5002
pause
