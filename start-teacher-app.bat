...
if not exist "lib\main_teacher.dart" (
  echo [ERROR] lib\main_teacher.dart not found - is mobile-app in the right place?
  pause
  exit /b 1
)
...
echo Freeing port 5001 if already in use...
for /f "tokens=5" %%a in ('netstat -aon ^| findstr ":5001 "') do taskkill /F /PID %%a >nul 2>&1

echo Starting Teacher app (Flutter Web) on http://localhost:5001 ...
call "%FLUTTER%" run -t lib\main_teacher.dart -d chrome --web-port=5001
pause
